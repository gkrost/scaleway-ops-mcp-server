...
- **#75 — copies above 5 GB; bucket-to-bucket prefix sync.** `scaleway_s3_copy_object` HEADs the source and, above S3's single-request 5 GB ceiling, switches to a multipart copy (512 MiB ranged `UploadPartCopy` parts — 10,000 × 512 MiB covers S3's 5 TB object maximum — aborted on failure so no billed parts leak); the result names the path taken. New `scaleway_s3_copy_prefix` does `rclone copy --ignore-existing` server-side: dry-run by default, skips existing keys, bounded by `max_objects` with a continuation token; `ignore_existing=false` requires confirm.
