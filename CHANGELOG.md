...
- `scaleway_s3_put_bucket_tagging`, `scaleway_s3_put_object_tags`, and `scaleway_s3_put_bucket_cors` now require `confirm=true` because they full-replace the existing tag/CORS set; omitted tags or rules are dropped. `put_bucket_cors` is also annotated `destructiveHint: true` (#48).
